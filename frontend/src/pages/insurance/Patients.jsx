...
import { CANONICAL_PROVIDERS, CANONICAL_PATIENTS, CANONICAL_FRAUD_DIAGNOSES, CANONICAL_REFERENCE } from '../../data/canonicalData';

function generateFallbackClaims(count = CANONICAL_REFERENCE.totalPatients) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    results.push({
      ...
      amount: Math.round(2500 + Math.random() * 5000),
...
function isHighRiskByRatio(p) {
  const totalClaims = p.total_claims || p.claim_count || 0;
  const fraudCount = p.fraud_count || 0;
  if (totalClaims === 0) return false;
  return (fraudCount / totalClaims) > 0.30;
}
...
export default function PatientManagement() {
  ...
  useEffect(() => {
    ...
    const fetchClaims = async () => {
      ...
      try {
        ...
        const fallback = generateFallbackClaims(CANONICAL_REFERENCE.totalPatients);
  ...
  const filtered = useMemo(() => {
    return patients.filter(p => {
      ...
      const highRisk = isHighRiskByRatio(p);
      const matchesRisk =
        riskFilter === 'All' ||
        (riskFilter === 'High Risk' && highRisk) ||
        (riskFilter === 'Normal' && !highRisk);
  ...
  const stats = useMemo(() => {
    ...
    const highRisk = patients.filter(p => isHighRiskByRatio(p)).length;
  ...
  const claimsChartData = useMemo(() => {
    ...
    patients.forEach(p => {
      ...
      if (c <= 1) dist['1'] += 1;
      else if (c <= 3) dist['2-3'] += 1;
      else if (c <= 5) dist['4-5'] += 1;
      else if (c <= 10) dist['6-10'] += 1;
      else dist['11+'] += 1;
    });
    return [{
      x: Object.keys(dist), y: Object.values(dist), type: 'bar',
      marker: { color: ['#14b8a6', '#0d9488', '#0f766e', '#115e59', '#064e3b'], line: { width: 0 } },
  ...
  const exportCSV = () => {
    ...
    const csvContent = [
      ...
      ...filtered.map(p => [
        ...
        isHighRiskByRatio(p) ? 'High Risk' : 'Normal'
      ].join(','))
    ].join('\n');
      ...
      <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-textSecondary">Distribution of patient risk ratings based on fraud claims / total claims ratio (&gt;30% = High Risk)</p>
          </div>
        ...
        <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-textSecondary">Detected behavioral patterns indicative of potential fraud — multi-provider visits, rapid claim filing, and upcoding rings</p>
            </div>
      ...
      <div className="enterprise-card">
        <div className="flex items-center gap-3 border-b border-border p-4 flex-wrap">
          ...
          >
            ...
            <option value="High Risk">High Risk (Fraud &gt;30%)</option>
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                paginated.map((patient) => {
                  ...
                  const highRisk = isHighRiskByRatio(patient);
                  const avgClaim = patient.avg_claim_amount || 0;
                  const pv = patient.providers_visited || 1;
                  const fraudCount = patient.fraud_count || 0;
                  const totalClaims = patient.total_claims || patient.claim_count || 0;
                  const fraudRatio = totalClaims > 0 ? fraudCount / totalClaims : 0;
                  const displayRiskLabel = highRisk ? 'High' : riskScore >= 0.2 ? 'Medium' : 'Low';
                  const displayRisk = highRisk
                    ? { label: 'High', color: 'text-orange-500', bg: 'bg-orange-500/10', border: 'border-orange-500/20' }
                  ...
                  return (
                    <tr key={patient.patient_id} className="hover:bg-bg/50 transition-colors">
                      ...
                      <td>
                        <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider ${displayRisk.bg} ${displayRisk.color} border ${displayRisk.border}`}>
                          {highRisk ? <AlertTriangle size={10} /> : <ShieldCheck size={10} />}
                          {displayRisk.label}
                          {highRisk && <span className="text-[8px] font-mono ml-0.5">({(fraudRatio * 100).toFixed(0)}%)</span>}
                        </span>
                      </td>
                      <td>
                        <button
              ...
              <div className="flex items-start gap-5">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 mt-2 flex-wrap">
                    {(() => {
                      const highRisk = isHighRiskByRatio(selectedPatient);
