...
from services.kafka_producer import send_claim_to_kafka
router = APIRouter()
...
async def process_claim(claim: dict, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    ...
    try:
        send_claim_to_kafka({**claim, "fraud_score": prediction['fraud_score']})
    except Exception as e:
        print(f"⚠️ Kafka send failed (not critical): {e}")
